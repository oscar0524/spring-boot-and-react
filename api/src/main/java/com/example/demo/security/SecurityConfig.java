...
public class SecurityConfig {
     ...
     */
    @Bean
    @Order(Ordered.HIGHEST_PRECEDENCE + 1) // 設置過濾器的優先級
    SecurityWebFilterChain springJWTRefreshTokenSecurityFilterChain(
            ServerHttpSecurity http,
            JwtRefreshTokenAuthFilter jwtFilter,
            CorsWebFilter corsWebFilter) {

        return http
                // 設定此過濾器鏈處理除了 /form/** 以外的所有請求
                .securityMatcher(ServerWebExchangeMatchers.pathMatchers("/user/token"))
                // 禁用CSRF保護
                .csrf(ServerHttpSecurity.CsrfSpec::disable)
                // 禁用HTTP基本認證
                .httpBasic(ServerHttpSecurity.HttpBasicSpec::disable)
    ...
    SecurityWebFilterChain springJWTSecurityFilterChain(
            ServerHttpSecurity http,
            JwtAccessTokenAuthFilter jwtFilter,
            CorsWebFilter corsWebFilter) {
